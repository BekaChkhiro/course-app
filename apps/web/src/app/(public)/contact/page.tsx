...
import { useSiteSettings, usePageContent } from '@/hooks/useSiteSettings';
...
export default function ContactPage() {
  ...
  const formatPhone = (phone: string | undefined) => phone || '+995 596 89 91 91';
  ...
  const heroTitle = pageContent?.heroTitle || 'დაგვიკავშირდი';
  const heroSubtitle = pageContent?.heroSubtitle || 'გვაქვს კითხვა? მოგვწერე და დაგეხმარებით';
                ...
                <div className="flex space-x-4">
                  {socialLinks.map((social, index) => (
                    <a
                      ...
                      className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center text-primary-900 hover:bg-primary-200 transition-colors"
                    >
                      {social.icon}
                    </a>
                  ))}
